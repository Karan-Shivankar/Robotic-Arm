...
<html>
  ...
  <body class="noselect" align="center" style="background-color:white">
    ...
    <h1 style="color: teal;text-align:center;">Karan KYZER</h1>
...
void moveServoSlowly(int servoIndex, int targetPosition) {
  int currentPosition = servoPins[servoIndex].servo.read();
  int step = (currentPosition < targetPosition) ? 1 : -1;
  
  // Speed control - lower number = slower movement
  int delayBetweenSteps = 20; // milliseconds between position updates
  
  while (currentPosition != targetPosition) {
    currentPosition += step;
    servoPins[servoIndex].servo.write(currentPosition);
...
void writeServoValues(int servoIndex, int value) {
  if (recordSteps) {
    ...
    if (recordedSteps.size() == 0) {
      // We will first record initial position of all servos.
      for (int i = 0; i < servoPins.size(); i++) {
  ...
  // Use the slow movement function instead of instantly changing position
  moveServoSlowly(servoIndex, value);
  
  // Update the UI
  wsRobotArmInput.textAll(servoPins[servoIndex].servoName + "," + value);
}
